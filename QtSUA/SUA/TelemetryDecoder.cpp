...
TelemetryPacket TelemetryDecoder::GetTelemetryPacket(bool* ok)
{
  ...
  if (isSynchonized && inputBytes.count() > 214)
  {
    ...
    try
    {
      ...
      unsigned char workMode_char = (unsigned char)arr[189];
      int workMode = (int)workMode_char;
      //workmode - QString от int в двоичной системе координат;
      packet.workMode = QString::number(workMode, 2);
      packet.workMode = packet.workMode.rightJustified(8, '0', true);
      unsigned char error_char = (unsigned char)arr[191];
      int error = (int)error_char;
      packet.error = QString::number(error, 2);
      packet.error = packet.error.rightJustified(8, '0', true);
